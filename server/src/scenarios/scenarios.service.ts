...
import type { Service, ServiceName } from './types/service.type';
import GithubIntegration from './integrations/github.integration';

@Injectable()
export class ScenariosService {
  private integrations = [new TwitterIntegration(), new GithubIntegration()];
  ...
  async emit(
    ...
    for (let { name, value } of reaction.fields) {
      value = value.replace(
        /{{(.*?)}}/g,
        (_, p1) => trigger.ingredients.find((x) => x.name === p1)?.value ?? '',
      );
  ...
  async createScenario(
    account: Account,
    createdScenario: ScenarioReqDto,
  ): Promise<Scenario> {
  ...
  async updateScenario(
    accountId: string,
    id: string,
    updatedScenario: ScenarioReqDto,
